...
try {
    ...
    if ($UnitTests -or $All) {
        ...
        $unitTestProjects = Get-ChildItem -Path "./Source/dbmig" -Filter "*Tests.csproj" -Recurse | Where-Object { $_.Name -notmatch "Integration" }
    ...
    if ($IntegrationTests -or $All) {
        ...
        $integrationTestProjects = Get-ChildItem -Path "./Source/dbmig" -Filter "*IntegrationTests.csproj" -Recurse
